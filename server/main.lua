...
local function giveStarterItems()
    ...
    for _, v in pairs(QBCore.Shared.StarterItems) do
        ...
        exports['qb-inventory']:AddItem(source, v.item, 1, false, info, 'dach-cityhall:giveStarterItems')
    end
end

-- Callbacks

QBCore.Functions.CreateCallback('dach-cityhall:server:receiveJobs', function(_, cb)
    cb(availableJobs)
end)

-- Events

RegisterNetEvent('dach-cityhall:server:requestId', function(item, hall)
    local src = source
    ...
    if not exports['qb-inventory']:AddItem(source, item, 1, false, info, 'dach-cityhall:server:requestId') then return end
    if item == 'weaponlicense' then
...
RegisterNetEvent('dach-cityhall:server:sendDriverTest', function(instructors)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    for i = 1, #instructors do
        ...
        if SchoolPlayer then
            TriggerClientEvent('dach-cityhall:client:sendDriverEmail', SchoolPlayer.PlayerData.source, Player.PlayerData.charinfo)
        else
            local mailData = {
                sender = 'Township',
...
RegisterNetEvent('dach-cityhall:server:ApplyJob', function(job, cityhallCoords)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    local jobLabel = JobInfo and JobInfo.label or availableJobs[job].label or job
    Player.Functions.SetJob(data.job, 0)
    TriggerClientEvent('QBCore:Notify', data.src, Lang:t('info.new_job', { job = jobLabel }))
end)

RegisterNetEvent('dach-cityhall:server:getIDs', giveStarterItems)
